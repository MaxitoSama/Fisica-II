...
ModuleCamera3D::ModuleCamera3D(Application* app, bool start_enabled) : Module(app, start_enabled)
{
	...
	Position = vec3(0.0f, 0.0f, 2.828427125f);
...
update_status ModuleCamera3D::Update()
{
	...
	{
		App->camera->Move({ 0,0.01f,0 });
		App->camera->Reference+={ 0,0.01f,0 };
	}
	...
	if (App->input->GetKey(SDL_SCANCODE_S) == KEY_REPEAT)
	{
		...
		App->camera->Reference += { 0, 0, 0.01f };
	}
	...
	{
		App->camera->Move({ -0.01f,0,0 });
		App->camera->Reference += { 0.01f, 0, 0 };
	}
